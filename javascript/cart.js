document.addEventListener("DOMContentLoaded", () => {
    ...
    // Quantity Increment/Decrement handler for cart rows
    const quantityContainers = document.querySelectorAll(".quantity-minus, .quantity-plus");
    quantityContainers.forEach(button => {
        button.addEventListener("click", (e) => {
            ...
            if (e.target.classList.contains("quantity-plus")) {
                value += 1;
            } else if (e.target.classList.contains("quantity-minus") && value > 1) {
                value -= 1;
            }
            input.value = value;

            // Trigger subtotal update
            updateRowSubtotal(container.closest("tr"), value);
        });
    });

    // Subtotal calculations when qty changes
    const updateRowSubtotal = (row, qty) => {
        if (!row) return;
        const priceCell = row.querySelector(".text-primary");
        const subtotalCell = row.querySelector(".product-subtotal, .font-bold.text-primary");
